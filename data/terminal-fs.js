...
(function () {
  ...
  if (EDUCATION.degrees.length) {
    ...
    lines.push(['cd', '  Education details available on request.']);
    lines.push(['tb', '']);
  }
  ...
  if (issuers.length) {
    ...
    lines.push(['cd', '  No certifications listed.']);
    lines.push(['tb', '']);
  }
...
(function () {
  const responseTime = PERSONAL.responseTime || 'usually within 24 hours';
  const lines = [
    ['cg', '# contact.md'],
    ['tb', ''],
  ];
  ...
  if (PERSONAL.email) {
    lines.push(['cd', 'Email    :']);
    lines.push(['lk', 'mailto:' + PERSONAL.email]);
  }
  if (PERSONAL.phone) {
    lines.push(['cd', 'Phone    :']);
    lines.push(['lk', 'tel:' + PERSONAL.phone.replace(/[^+\d]/g, '')]);
  }
  if (PERSONAL.github) {
    lines.push(['cd', 'GitHub   :']);
    lines.push(['lk', 'https://' + PERSONAL.github]);
  }
  if (PERSONAL.linkedin) {
    lines.push(['cd', 'LinkedIn :']);
    lines.push(['lk', 'https://' + PERSONAL.linkedin]);
  }

  lines.push(['tb', '']);
  lines.push(['cg', '● ' + PERSONAL.statusText]);
  lines.push(['tb', '']);
  lines.push(['cd', 'Response time: ' + responseTime + '.']);

  TERMINAL_FS['portfolio'].children['contact.md'].content = lines;
})();
...
(function () {
  ...
  dir['readme.md'].content = [
    ['cg', '# readme.md'],
    ['tb', ''],
    ['cw', 'Welcome to ' + PERSONAL.name + "'s portfolio terminal."],
    ['tb', ''],
